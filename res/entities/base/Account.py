...
class Account(shyloo.Proxy):
	"""
	账号实体
	客户端登陆到服务端后，服务端将自动创建这个实体，通过这个实体与客户端进行交互
	"""
	def __init__(self):
		shyloo.Proxy.__init__(self)
	...
	def reqCreateAvatar(self, roleType, name):
		...
		avatar = shyloo.createBaseLocally('Avatar', props)
		if avatar:
	...
	def onLogOnAttempt(self, ip, port, password):
		...
		if self.activeAvatar != None:
			return shyloo.LOG_ON_REJECT

		if ip == self.lastClientIpAddr and password == self.password:
			return shyloo.LOG_ON_ACCEPT
		else:
			return shyloo.LOG_ON_REJECT
		"""
