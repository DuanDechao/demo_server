...
class Space(shyloo.Entity, GameObject):
	"""
	游戏场景，在这里代表野外大地图
	"""
	def __init__(self):
		...
		#shyloo.addSpaceGeometryMapping(self.spaceID, None, resPath, True, {0 : "srv_xinshoucun_1.navmesh", 1 : "srv_xinshoucun.navmesh"})
		shyloo.addSpaceGeometryMapping(self.spaceID, None, resPath)
		
		DEBUG_MSG('created space[%d] entityID = %i, res = %s.' % (self.spaceUType, self.id, resPath))
		
		shyloo.globalData["space_%i" % self.spaceID] = self.base
	...
	def onDestroy(self):
		...
		shyloo method.
		"""
		del shyloo.globalData["space_%i" % self.spaceID]
		self.destroySpace()
